...
type HttpFilter struct {
	...
	defaultAuth  synthesis.AuthWrapper //defaultAuth is the default auth wrapper for any routes. It is used to generate authorized RawRoute.
...
func NewFilter(authorizer ...synthesis.AuthWrapper) *HttpFilter {
	var auth synthesis.AuthWrapper
	if authorizer == nil || authorizer[0] == nil {
		auth = synthesis.AuthAll(authorizer...)
	} else {
		auth = synthesis.NoAuth
	}
	return &HttpFilter{
		origin:       "",
		customHeader: "",
